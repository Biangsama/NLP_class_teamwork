...
class BertBiLSTM(nn.Layer):
    ...
    def forward(self, input_ids, segment_ids):
        ...
        sequence_output, _ = self.bert(input_ids, segment_ids)
...
if __name__ == "__main__":
    ...
    model = BertBiLSTM(hidden_dim=384, output_size=2, n_layers=2, bidirectional=True, drop_prob=0.5)
    ...
    #     paddle.save(model.state_dict(), f'./saved_models/model_epoch_{epoch}.pdparams')

    def predict(model, data, tokenizer, label_map, batch_size=1, max_seq_length=256):
        """
        Predicts the data labels.

        Args:
            model (obj:`paddle.nn.Layer`): A model to classify texts.
            data (obj:`List[str]`): The processed data whose each element is a text string.
            tokenizer (obj:`PretrainedTokenizer`): Tokenizer to convert the text to token IDs.
            label_map (obj:`dict`): The label id (key) to label str (value) map.
            batch_size (obj:`int`, defaults to 1): The number of batch.
            max_seq_length (obj:`int`, defaults to 256): The maximum sequence length.

        Returns:
            results (obj:`list`): All the predicted labels.
        """
        examples = []
        for text in data:
    ...
    model_state_dict = paddle.load('./bert_lstm/model_epoch_3.pdparams')
    model.set_state_dict(model_state_dict)
    # Set model to evaluation mode
    model.eval()
    # Data to predict
    data = ["这是一条测试文本", "老师我今天想请一个假已经和辅导员说过了", "服务态度很差非常不满意"]
    # Perform prediction
    predictions = predict(model, data, tokenizer, label_map, batch_size=2)
    # Print predictions
